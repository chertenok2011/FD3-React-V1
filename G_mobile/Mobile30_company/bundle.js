...
var MobileCompany = function (_React$Component) {
  ...
  function MobileCompany() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = MobileCompany.__proto__ || Object.getPrototypeOf(MobileCompany)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
      ...
      if (newState.name != _this.state.name) return true; // если название компании изменилось - перерисовываем
      if (newState.clients.length != _this.state.clients.length) return true; // если количество клиентов изменилось - перерисовываем
      var clientsChanged = false; // из forEach нельзя сделать return - это будет return из функции forEach
      newState.clients.forEach(function (newClient, i) {
        var oldClient = _this.state.clients[i];
        if (oldClient.fio != newClient.fio) clientsChanged = true; // если фамилия клиента изменилась - перерисовываем
        if (oldClient.balance != newClient.balance) clientsChanged = true; // если баланс клиента изменился - перерисовываем
        /*
